...
const load = async ({ params, locals: { safeGetSession } }) => {
  ...
  const { data: piece, error: pieceError } = await supabase.from("pieces").select("id, name, description, ai_description, created_at, updated_at, cover_image_id").eq("id", params.id).eq("user_id", user.id).single();
  if (pieceError || !piece) error(404, "Piece not found");
  const { data: images, error: imagesError } = await supabase.from("images").select("id, piece_id, user_id, storage_path, uploaded_at, notes, is_cover").eq("piece_id", params.id).eq("user_id", user.id).order("uploaded_at", { ascending: true });
